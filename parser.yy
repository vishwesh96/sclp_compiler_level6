...
procedure_declaration:
	...
	if(NOT_ONLY_PARSE)
	{
		...
		if (st == NULL)
			st = new Symbol_Table();
		proc->set_formal_list(*st);			//if no parameters means NULL empty formal symbol table
		$$ = proc;
...
| 
	...
	if(NOT_ONLY_PARSE)
	{
		Ast * actual_parameter = $3;
		std::list<Ast *> *actual_parameters_list = $1;
...
function_call:
	...
	if(NOT_ONLY_PARSE)
	{
		...
		if(current_procedure!=NULL){
					CHECK_INPUT ((current_procedure->variable_in_symbol_list_check(*fname) == false),
				"Called Procedure name cannot be same as local variable name", get_line_number());
		}
...
argument_declaration:
	...
	if(NOT_ONLY_PARSE)
	{
		...
		// if(current_procedure!=NULL){
		// 	CHECK_INPUT((*name!=current_procedure->get_proc_name()),"Procedure name cannot be same as formal parameter name",get_line_number());			
		// }
